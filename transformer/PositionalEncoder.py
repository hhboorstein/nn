...
class PositionalEncoder(nn.Module):
    ...
    def __init__(self, d_model: int, context_len: int = 4096, p_dropout: float = 0.05):
        """
        ...
        # encoding tensor, compatible with input of shape (batch_size, context_len, emb_dim)
        PE = torch.zeros(1, context_len, d_model)
    ...
    def forward(self, x):
        """
        Args:
          x: Expects a tensor of shape (batch_size, seq_len, emb_dim)
        Returns: encoded tensor of shape (batch_size, seq_len, emb_dim)
        """
